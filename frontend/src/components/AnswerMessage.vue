    ...
    <v-card>
      ...
      <v-card-text>
        <v-form name="form" v-model="valid" ref="answerForm">
          <v-text-field label="Titre de votre message *" name="title" prepend-icon="mdi-message-text-outline" type="text" id="title" v-model="message.title" counter :rules="titleRules"></v-text-field>
          <v-row class="caption ml-2">Exemple : Détente et gourmandise ! </v-row>

          <v-textarea filled auto-grow label="Votre message *" name="content" prepend-icon="mdi-message-text-outline" type="text" id="content" v-model="message.content" counter :rules="contentRules" />
